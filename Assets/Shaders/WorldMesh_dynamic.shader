Shader "Custom/WorldMesh_dynamic" {
   Properties {
   		  _ScaleTex ("_ScaleTex", 2D) = "white" {}

   		 // _AlbedoTextures("Albedo Textures", 2DArray) = "" {}
   		  //_NormalTextures("Albedo Textures", 2DArray) = "" {}
    ...
    SubShader 
    {
	      ...
	      void surf (Input IN, inout SurfaceOutputStandard o) 
	      {
			 ...
			 float2 uv_Scaled = IN.uv_ScaleTex *_TexScaleFactor + int2(_UVWorldOffsetX, _UVWorldOffsetZ);
	         ...
	         else if(_WorldIndexToTexArray[D_Texture_Index] == 2) // resolution 2048 x 2048
	         {
	      	...
	      	//A_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures512, float3(IN.uv_ScaleTex, 0));
	       	//B_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures512, float3(IN.uv_ScaleTex, 0));
	      	//C_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures512, float3(IN.uv_ScaleTex, 0));
	      	//D_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures512, float3(IN.uv_ScaleTex, 0));

	      	 //A_Normal = UnpackNormal(UNITY_SAMPLE_TEX2DARRAY(_NormalTextures512, float3(IN.uv_ScaleTex, 0)));
	         // B_Normal = UnpackNormal(UNITY_SAMPLE_TEX2DARRAY(_NormalTextures512, float3(IN.uv_ScaleTex, 0)));
	         // C_Normal = UnpackNormal(UNITY_SAMPLE_TEX2DARRAY(_NormalTextures512, float3(IN.uv_ScaleTex, 0)));
	          //D_Normal = UnpackNormal(UNITY_SAMPLE_TEX2DARRAY(_NormalTextures512, float3(IN.uv_ScaleTex, 0)));

	         //A_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures, float3(IN.uv_ScaleTex, A_Texture_Index));
	         //B_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures, float3(IN.uv_ScaleTex, B_Texture_Index));
	         //C_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures, float3(IN.uv_ScaleTex, C_Texture_Index));
	         //D_Color = UNITY_SAMPLE_TEX2DARRAY(_AlbedoTextures, float3(IN.uv_ScaleTex, D_Texture_Index));

	         //A_Normal = UnpackNormal( UNITY_SAMPLE_TEX2DARRAY(_NormalTextures, float3(IN.uv_ScaleTex, A_Texture_Index)));
	         //B_Normal = UnpackNormal( UNITY_SAMPLE_TEX2DARRAY(_NormalTextures, float3(IN.uv_ScaleTex, B_Texture_Index)));
	         //C_Normal = UnpackNormal( UNITY_SAMPLE_TEX2DARRAY(_NormalTextures, float3(IN.uv_ScaleTex, C_Texture_Index)));
	         //D_Normal = UnpackNormal( UNITY_SAMPLE_TEX2DARRAY(_NormalTextures, float3(IN.uv_ScaleTex, D_Texture_Index)));

	         //if(A_Texture_Index == 0)
	         //{
	         //	A_Color = tex2D (_Tex1, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw) ).rgb;
	         //	A_Normal = UnpackNormal (tex2D (_Tex1Normal, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw)));
	         //}

	         //if(B_Texture_Index == 0)
	         //{
	         //	B_Color = tex2D (_Tex1, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw) ).rgb;
	         //	B_Normal = UnpackNormal (tex2D (_Tex1Normal, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw)));
	         //}

	         //if(C_Texture_Index == 0)
	         //{
	         //	C_Color = tex2D (_Tex1, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw) ).rgb;
	         //	C_Normal = UnpackNormal (tex2D (_Tex1Normal, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw)));
	         //}

	         //if(D_Texture_Index == 0)
	         //{
	         //	D_Color = tex2D (_Tex1, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw) ).rgb;
	         //	D_Normal = UnpackNormal (tex2D (_Tex1Normal, (IN.uv_ScaleTex * _Tex1_ST.xy + _Tex1_ST.zw)));
	         //}
	         ...
	         float UV_2Vertex_x = (IN.uv_ScaleTex.x * 1024 - indexX_A_FloatArray * 78.77) / 78.77; // 1024 / 13 = 78.77
	         float UV_2Vertex_y = (IN.uv_ScaleTex.y * 1024 - (indexY_A_FloatArray/13.0) * 78.77) / 78.77; // 1024 / 13 = 78.77
