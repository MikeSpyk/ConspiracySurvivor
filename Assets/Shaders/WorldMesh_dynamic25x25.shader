Shader "Custom/WorldMesh_dynamic25x25" {
   Properties {
    ...
    SubShader 
    {
	      ...
	      float _VertexTextureMap[625]; // _VertexCountEdge * _VertexCountEdge
	      ...
	      {
	         int indexX_A_FloatArray = IN.uv_ScaleTex.x * 25 ;
	         int indexY_A_FloatArray = (int)(IN.uv_ScaleTex.y * 25) * 25;

	         float A_Texture_Index = _VertexTextureMap[indexX_A_FloatArray + indexY_A_FloatArray];
	         float B_Texture_Index = _VertexTextureMap[indexX_A_FloatArray + (int)((IN.uv_ScaleTex.y * 25) +1) * 25];
	         float C_Texture_Index = _VertexTextureMap[indexX_A_FloatArray +1+ (int)((IN.uv_ScaleTex.y * 25) +1) * 25];
			 ...
			 float2 uv_Scaled = IN.uv_ScaleTex *_TexScaleFactor * 25 + int2(_UVWorldOffsetX, _UVWorldOffsetZ);
	         ...
	         // vertexCount on edge is 25
	         // pixel-count between two vertices is 81.92
	         ...
	         float UV_2Vertex_x = (IN.uv_ScaleTex.x * 2048 - indexX_A_FloatArray * 81.92) / 81.92;
	         float UV_2Vertex_y = (IN.uv_ScaleTex.y * 2048 - (indexY_A_FloatArray/25.0) * 81.92) / 81.92;
