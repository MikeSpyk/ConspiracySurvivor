...
public class ShortCompressedFloat
{
    ...
    public void compress()
    {
        ...
        m_multiplier = maxValue / ushort.MaxValue;

        m_compressedArray = new ushort[m_inputArray.GetLength(0), m_inputArray.GetLength(1)];

        for (int i = 0; i < m_inputArray.GetLength(0); i++)
        {
            for (int j = 0; j < m_inputArray.GetLength(1); j++)
            {
                m_compressedArray[i, j] = (ushort)Mathf.Max(0, m_inputArray[i, j] / m_multiplier - m_offset);
            }
        }
